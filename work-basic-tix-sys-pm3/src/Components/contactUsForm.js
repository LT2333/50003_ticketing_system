...
class ContactUs extends Component {
  ...
  toggle() {
    ...
    var options = { method: 'POST',
      url: 'https://courier50003.herokuapp.com/portal/profile-img-upload',
      headers: 
      { 
      ...
      formData: 
      { profileImage: 
          { value: fs.createReadStream("picXYY.jpg"),
            options: { filename: 'picXYY.jpg', contentType: null } } } };
  ...
  //For uploading the file through "file"
	...
	singleFileUploadHandler = ( event ) => {
	  ...
	  // If file selected
		if ( this.state.selectedFile ) {
	  //data.append( 'profileImage', this.state.selectedFile, this.state.selectedFile.name );  
	  axios.post( 'http://localhost:5000/api/profile-img-upload', this.state.selectedFile, {
		  headers: {
		   'accept': 'application/json',
		   'Accept-Language': 'en-US,en;q=0.8',
		   }              //'Content-Type': `multipart/form-data; boundary=${data._boundary}` 
